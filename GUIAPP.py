...
class App(ctk.CTk):
    ...
    def setMascotaApp(self, mascotaUtilizar):
...
class screenBuscarMascota(ctk.CTkFrame):
    ...
    def clickBuscar(self, parent, container):
        ...
        if(resultado[0] == "MalCodigo"):
            ...
            mascotaMostrar = resultado[1] #Resultado 1 es el id de la mascota
            datosMascota = terminalVet.getDatosBasicosMascota(mascotaMostrar)
            print(datosMascota)
            self.setMascotaBuscada(datosMascota, parent)
            
            mascotaAcutal:Mascota = parent.getMascotaApp()
            ...
            infoBasica = f'Codigo mascota:  {str(mascotaAcutal.getId())} , Nombre mascota:  {str(mascotaAcutal.getNombreMascota())} , Especie:   {str(mascotaAcutal.getEspecie())} \nRaza:   {str(mascotaAcutal.getRaza())}  , Dueño/a:   {str(mascotaAcutal.getNombreTutor())}'
            ...
            infoBasica = f'Codigo mascota:  {mascotaAcutal.getId()} , Nombre mascota:  {str(mascotaAcutal.getNombreMascota())} , Especie:   {str(mascotaAcutal.getEspecie())} \nRaza:   {str(mascotaAcutal.getRaza())}  , Dueño/a:   {str(mascotaAcutal.getNombreTutor())}'
            self.labelInfoBasica = ctk.CTkLabel(self.frameMensajes, justify='left', text=infoBasica, text_font=Font_tuple, text_color='black', width=100)
            self.labelInfoBasica.grid(row=1, column=0, padx=10, pady=30)
    ...
    def setMascotaBuscada(self, datosMascota, parent):
        sql = 'SELECT * FROM RegistroDeOperaciones WHERE TablaMedica_idTablaMedica = (%s)'
        mycursor.execute(sql, (str(datosMascota[9]),)) #el numero 9 representa el campo 10 de la tabla de mascotas = id tabla medica 
        registroOp = mycursor.fetchall()
        sql = 'SELECT * FROM RegistroVacunasSuministradas WHERE TablaMedica_idTablaMedica = (%s)'
        mycursor.execute(sql, (str(datosMascota[9]),))
        ...
        tablaEntregar = TablaMedica(datosMascota[9])
        ...
        mascotaBuscada = Mascota(datosMascota[0], datosMascota[1], datosMascota[2], datosMascota[3], datosMascota[4], datosMascota[5], datosMascota[6],
                datosMascota[7], datosMascota[8], tablaEntregar)

        parent.setMascotaApp(mascotaBuscada)
...
class screenFormularioAgregarMascota(ctk.CTkFrame):
    def __init__(self, parent, container):
        ...
        self.botonAgregarMascota = ctk.CTkButton(self.frameButtonsSAgregarMascota, width= 200, height= 120,text='Agregar Mascota', hover_color="#142C3D", command=lambda: self.clickAgregarMascota(parent,
            ...
            self.entradaRutTutorSAgregarMascota.get(), self.entradaNumeroTelefonoSAgregarMascota.get(), self.entradaDireccionTutorSAgregarMascota.get(), self.entradaAlergiasSAgregarMascota.get()))
    ...
    def clickAgregarMascota(self, parent, nombreMascota, especie, color, raza, nombreTutor, rutTutor, numTel, alergias, direccion):

        idNuevaMascota = parent.getIdNuevaMascota()
        tablaMedica = TablaMedica(uuid.uuid4())
        terminalVet.agregarMascota(idNuevaMascota, nombreMascota, especie, color, raza, nombreTutor, rutTutor, numTel, direccion, alergias, tablaMedica)
